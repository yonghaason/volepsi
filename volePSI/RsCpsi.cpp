...
namespace volePSI
{
    ...
    Proto RsCpsiSender::sendNew(span<block> Y, oc::MatrixView<u8> values, Sharing &ret, Socket &chl)
    {
        MC_BEGIN(Proto, this, Y, values, &ret, &chl,
                 cuckooSeed = block{},
                 params = oc::CuckooParam{},
    ...
    Proto RsCpsiReceiver::receiveNew(span<block> X, Sharing &ret, Socket &chl)
    {
        MC_BEGIN(Proto, this, X, &ret, &chl,
                 cuckooSeed = block{},
                 cuckoo = oc::CuckooIndex<>{},
                 ...
                 count = u64{},
                 cuckooT = std::vector<block>{},
                 TyTheir = std::vector<block>{},
                 TvTheir = Matrix<u8>{},
                 TyTheirSet = std::set<block>{},
                 TabToIdx = std::map<block, u64>{},
                 tmpblkk = block{}
                );
