...
namespace volePSI
{
    ...
    Proto SilentTripleGen::expand(coproto::Socket& chl)
    {
        MC_BEGIN(Proto,this, &chl,
            ...
            j = u64{},
            sendMsg = std::vector<std::array<block, 2>>{},
            recvMsg = std::vector<block>{},
            recvOtChoiceBits = oc::BitVector{},
        ...
        if (mSenderOT.size())
        {
            ...
            {
                MC_AWAIT(mSenderOT[j].silentSend(sendMsg, mPrng, chl));
        ...
        else
        {
            ...
            for (j = 0; j < mRecverOT.size(); ++j)
            {
                ...
                MC_AWAIT(mRecverOT[j].silentReceive(recvOtChoiceBits, recvMsg, mPrng, chl));
        ...
        oc::Timer* timer)
    {
        ...
        if (partyIdx)
        {
            MC_BEGIN(Proto,b = std::move(b), &prng, &chl, recvMsgP, sendMsgP, timer,
                base = oc::DefaultBaseOT{},
                msg = std::vector<std::array<block, 2>>(128)
            );
            ...
            MC_AWAIT(extend(b, msg, prng, chl, recvMsgP, sendMsgP));
        ...
        else
        {
            ...
            MC_BEGIN(Proto,b = std::move(b), &prng, &chl, recvMsgP, sendMsgP, timer,
                base = oc::DefaultBaseOT{},
                msg = std::vector<block>(128),
                bv = oc::BitVector(128)
            );

            bv.randomize(prng);

            MC_AWAIT(base.receive(bv, msg, prng, chl));
            if (timer)
                timer->setTimePoint("generateBase base");


            MC_AWAIT(extend(b, bv, msg, prng, chl, recvMsgP, sendMsgP));
    ...
    Proto extend(
        ...
        span<std::array<block, 2>> sendMsgP)
    {
        MC_BEGIN(Proto,b, baseMsg, &prng, &chl, recvMsgP, sendMsgP,
            recvMsg = std::vector<block>{},
            sendMsg = std::vector<std::array<block, 2>>{},
            sendBaseChoice = oc::BitVector{},
            sendBaseMsg = std::vector<block>{},
            recvOT = std::move(oc::IknpOtExtReceiver{}),
            sendOT = std::move(oc::IknpOtExtSender{})
        );
        ...
        if (recvMsg.size())
        {
            ...
            MC_AWAIT(recvOT.receive(b.mRecvChoiceBits, recvMsg, prng, chl));
        ...
        if (b.mNumSend)
        {
            ...
            if (sendBaseMsg.size())
                ...
                MC_AWAIT(sendOT.genBaseOts(prng, chl));

            MC_AWAIT(sendOT.send(sendMsg, prng, chl));
        }
        ...
        span<std::array<block, 2>> sendMsgP)
    {
        ...
        MC_BEGIN(Proto,b, baseChoice = std::move(baseChoice), baseMsg, &prng, &chl, recvMsgP, sendMsgP,
            recvMsg = std::vector<block>{},
            sendMsg = std::vector<std::array<block, 2>>{},
            recvBaseMsg = std::vector<std::array<block, 2>>{},
            recvOT = std::move(oc::IknpOtExtReceiver{}),
            sendOT = std::move(oc::IknpOtExtSender{})
        );
